...
J_VALUES=("Medium")
#("VVLoose" "Medium" "Loose" "Tight" "VTight" "VLoose") 
E_VALUES=("VVLoose")
#("Tight" "VVLoose") 
...
INPUT_ROOT="input_pt_less_region_corrTES_pnet"           # inputs are shared with the uncorrelated workflow
OUTPUT_ROOT="output_pt_less_region_corrTES_pnet"
...
for JET_WP in "${J_VALUES[@]}"; do
  for ELE_WP in "${E_VALUES[@]}"; do
    ...
    #   FD="${BASE_POSTFIT}/${YEAR}/fitDiagnostics.mt_m_vis-${DM}_mutau_PNet-${YEAR}-13TeV.root"
    ...
    # echo "=== Step 3: Plots ==="
    # python3 python/plot/runpostfit.py --variant corr -c $CONFIG_TT -j ${JET_WP} -e ${ELE_WP} -y $YEAR --tagger pnet --include-cr \
    #   2>&1 | tee step3_plots_corrTES_${JET_WP}_${ELE_WP}.log
    # python3 pre_post_plot_combiner.py --variant corr --scan_dir ${BASE_PLOTS}/$YEAR/ --jet_wp ${JET_WP} --ele_wp ${ELE_WP} --tagger pnet
    # python3 plot_measurements.py --variant corr --jet_wp ${JET_WP} --ele_wp ${ELE_WP} --year $YEAR --tagger pnet
