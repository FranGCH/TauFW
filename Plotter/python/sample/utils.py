...
def getsampleset(datasample,expsamples,sigsamples=[ ],**kwargs):
  ...
  era        = kwargs.get('era',        ""    )
  fpattern   = kwargs.get('fname',       None  ) # file name pattern, e.g. $PICODIR/$SAMPLE_$CHANNEL$TAG.root
  fpattern   = kwargs.get('fpattern',   fpattern) # alias
  ...
  if datasample:
    ...
    if len(fnames)==1:
      ...
      datasample = MergedSample(dname,'Data',data=True)
      for fname in fnames:
