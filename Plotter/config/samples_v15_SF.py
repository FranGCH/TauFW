...
def getsampleset(channel,era,**kwargs):
  ...
  if split and channel.count('tau')==1:
    ...
    if 'DM' in split: # split DY by decay modes
      samples.split('DY', [('ZTTDM0', ZTT+", h^{#pm}",                   GMR+" && decayModePNet_2==0"),
                           ('ZTTDM1', ZTT+", h^{#pm}h^{0}",              GMR+" && decayModePNet_2==1"),
                           ('ZTTDM2', ZTT+", h^{#pm}#pi^{0}#pi^{0}",     GMR+" && decayModePNet_2==2"),
                           ('ZTTDM10',ZTT+", h^{#pm}h^{#mp}h^{#pm}",     GMR+" && decayModePNet_2==10"),
                           ('ZTTDM11',ZTT+", h^{#pm}h^{#mp}h^{#pm}h^{0}",GMR+" && decayModePNet_2==11"),
                           ('ZL',GML),('ZJ',GMJ),])
    elif 'DY' in split:
